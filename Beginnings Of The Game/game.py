...
def remove_punct(text):
    ...
    'goSouTh'
    """
    filter_punctuation = re.sub(r'[^a-zA-Z0-9\s]', '', text)
    text = filter_punctuation
...
def normalise_input(user_input):
    ...
    for char in user_input:
        if (not char.isspace()) and (not char.isalpha()):
            char = str("")
        normalised_input = normalised_input + char

    return ((normalised_input).lower()).strip()
...
def is_valid_exit(exits, user_input):
    ...
    True
    """
    #if user_input in exits: return True
    #else: return False
...
def menu(exits):
    ...
    while True:
        ...
        if is_valid_exit(exits, user_input) != True:
            print("Unfortunately that input is invalid, please try again.")
        else:
            return user_input
...
def move(exits, direction):
    ...
    False
    """
    counter = [exits][0][direction]
    next_room = rooms[counter]
    return next_room
